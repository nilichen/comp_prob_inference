...
def compute_posterior(prior, likelihood, y):
    ...
    log_likelihood_n = log_prior + log_likelihood[y].sum(axis=0)
    log_answer = log_likelihood_n - scipy.misc.logsumexp(log_likelihood_n)
    #
...
#         print(posterior.argmax())
        ...
        MAP_ratings[i] = posterior.argmax()
    #
...
def compute_true_movie_rating_posterior_entropies(num_observations):
    ...
    posteriors, MAP_ratings = infer_true_movie_ratings(num_observations=num_observations)
...
def main():
    ...
    best_movies = np.argsort(MAP_ratings)[-10:]
    best_movies_name = [movie_data_helper.get_movie_name(movie_id) for movie_id in best_movies]
    # print(best_movies_name)

    avg_posterior_entropy = []
    for n in range(1, 201):
        # print(n)
        avg_posterior_entropy.append(compute_true_movie_rating_posterior_entropies(n).mean())

    plt.plot(range(1, 201), avg_posterior_entropy)
    #
